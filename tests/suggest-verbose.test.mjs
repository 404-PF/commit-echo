...
function configDirFor(home) {
  if (platform() === 'darwin') return join(home, 'Library', 'Application Support', 'commit-echo');
  if (platform() === 'win32') return join(home, 'AppData', 'Roaming', 'commit-echo');
...
test('suggestCommand prints verbose diagnostics when requested', async () => {
  ...
  execFileSync('git', ['config', 'user.email', 'verbose@example.com'], { cwd: repo });
  ...
  const server = createServer((req, res) => {
    if (req.url === '/chat/completions' && req.method === 'POST') {
      ...
      res.end(JSON.stringify({
        model: 'fixture-model',
        choices: [{ message: { content: '1. feat: add verbose diagnostics' } }],
      }));
      return;
    }
  ...
  try {
    ...
    assert.match(stdout, /Diagnostic info:/);
